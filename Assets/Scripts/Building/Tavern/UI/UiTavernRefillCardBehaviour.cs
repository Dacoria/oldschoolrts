...
public class UiTavernRefillCardBehaviour : MonoBehaviour
{
    ...
    {
        var itemSettings = ResourcePrefabs.Get().Single(x => x.ItemType == foodType);
    ...
    {
        var foodSettings = FoodConsumptionSettings.ItemFoodRefillValues.Single(x => x.ItemType == foodType);
