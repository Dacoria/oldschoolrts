...
public class NodeBasedEditor : EditorWindow
{
    ...
    private void OnClickAddNode(Vector2 mousePosition)
    {
        ...
        nodes.Add(new Node(mousePosition, 200, 135, nodeStyle, selectedNodeStyle,
            inPointStyle, outPointStyle, OnClickInPoint, OnClickOutPoint, OnClickRemoveNode,
            nodeCount, false, 0, null, "", "", SkillType.None));
        ++nodeCount;
    }
    ...
    private void SaveSkillTree()
    {
        for (int i = 0; i < nodes.Count; ++i)
        {
            nodes[i].skill.Name = nodes[i].skill.SkillType.ToString();
        }
        ...
        for (int i = 0; i < nodes.Count; ++i)
        {
            ...
            skillTree.skilltree[i].SkillDependencyIds = dependencies;
        }
        ...
        if (File.Exists(path))
        {
            ...
            if (File.Exists(path))
            {
                ...
                for (int i = 0; i < _skillTree.Length; ++i)
                {
                    ...
                    if (_skillTree[i].SkillDependencyIds.Length == 0)
                    {
                ...
                for (int i = 0; i < nodes.Count; ++i)
                {
                    for (int j = 0; j < nodes[i].skill.SkillDependencyIds.Length; ++j)
                    {
                        if (skillDictionary.TryGetValue(nodes[i].skill.SkillDependencyIds[j], out outSkill))
                        {
                            for (int k = 0; k < nodes.Count; ++k)
                            {
    ...
    private void LoadSkillCreateNode(Skill skill, Vector2 position)
    {
        ...
        nodes.Add(new Node(position, 200, 135, nodeStyle, selectedNodeStyle,
            inPointStyle, outPointStyle, OnClickInPoint, OnClickOutPoint, OnClickRemoveNode, 
            skill.SkillId, skill.IsUnlocked, skill.Cost, skill.SkillDependencyIds, skill.Name, skill.Description, skill.SkillType));
        ++nodeCount;
    }
}